...
export default function MuseoPage() {
  ...
  const techniques = useMemo(() => {
    ...
    works.forEach((work) => {
      const tecnica = extraerTecnica(work.descripcion);
      if (tecnica && tecnica !== "Sin técnica") {
        set.add(tecnica);
      }
    });

    return ["Todas", ...Array.from(set).sort(), "Sin técnica"];
  }, [works]);
  ...
  const filteredWorks = useMemo(() => {
    ...
    return works.filter((work) => {
      ...
      const matchesTechnique =
        selectedTechnique === "Todas" || tecnica === selectedTechnique;
    ...
    <div className="min-h-screen bg-[#f4f5f5] text-neutral-950 transition-colors duration-300 dark:bg-neutral-950 dark:text-white">
      <Shell className="py-8 lg:py-10">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-sm leading-relaxed text-neutral-600 dark:text-white/65 sm:text-base">
                Cada obra respira, ocupa su espacio y dialoga con las demás.
                Explora la colección por artista, título o técnica.
              </p>
            </div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-[26px] border border-neutral-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-neutral-900">
          ...
          <div className="rounded-[26px] border border-neutral-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-neutral-900">
            ...
            <p className="mt-1 text-sm text-neutral-500 dark:text-white/55">
              {Math.max(techniques.length - 1, 0)} categorías visibles
            </p>
          </div>
        ...
        <div className="mt-6 rounded-[34px] border border-neutral-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-neutral-900 sm:p-7">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-neutral-500 dark:text-white/55">
                Filtra las obras por óleo, dibujo, cerámica, grabado, acuarela u
                otras técnicas registradas.
              </p>
            </div>
            ...
            <input
              ...
              className="w-full rounded-full border border-neutral-300 bg-neutral-50 px-5 py-4 text-sm text-neutral-950 outline-none transition placeholder:text-neutral-400 focus:border-emerald-500 focus:bg-white dark:border-white/10 dark:bg-white/5 dark:text-white dark:placeholder:text-white/35 dark:focus:border-emerald-400 lg:w-[380px]"
            />
          </div>
            ...
            {filteredWorks.map((w) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute left-4 top-4 z-20 rounded-full bg-white/95 px-4 py-2 text-[11px] font-black uppercase tracking-[0.12em] text-neutral-950 shadow-sm">
                    {tecnica}
                  </div>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 z-20 p-5">
                    ...
                    <div className="mt-4 flex items-center justify-between gap-3">
                      ...
                      <span className="rounded-full bg-emerald-400/90 px-3 py-1 text-[11px] font-black text-black">
                        Técnica
                      </span>
                    </div>
                  </div>
